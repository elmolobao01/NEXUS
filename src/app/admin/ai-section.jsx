...
function BenchmarkPanel({ onError }) {
  ...
  return <section className="ai2-benchmark-grid">
    <article className="ai2-panel ai2-benchmark-runner">
      <header><div><span>BENCHMARK COMPETITIVO · v0.9.1</span><h3>Qualidade × custo × latência por capacidade</h3></div></header>
...
export default function AISection() {
  ...
  return (
    <div className="ai2-shell">
      ...
      {tab === "Modelos" && <section className="ai2-panel"><header><div><span>CATÁLOGO</span><h3>Modelos e custo de inferência</h3></div></header><div className="ai2-table-wrap"><table><thead><tr><th>Modelo</th><th>Provider</th><th>Nível</th><th>Input / 1M</th><th>Output / 1M</th><th>Referência / 1M</th><th>Modo</th><th>Status</th></tr></thead><tbody>{(data?.models || []).map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small>{item.code}</small></td><td>{item.provider?.name || providerMap[item.provider_id]?.name || "—"}</td><td><span className="ai2-level-pill">{levelName(item.level)}</span></td><td>{usd(item.input_cost_per_million, 3)}</td><td>{usd(item.output_cost_per_million, 3)}</td><td>{usd(item.reference_input_cost_per_million ?? item.input_cost_per_million,3)} / {usd(item.reference_output_cost_per_million ?? item.output_cost_per_million,3)}</td><td>{item.billing_mode || "PAID"}</td><td><div className="ai2-actions"><button onClick={() => { const input = window.prompt("Custo de entrada por 1M tokens (USD)", item.input_cost_per_million); if (input === null) return; const output = window.prompt("Custo de saída por 1M tokens (USD)", item.output_cost_per_million); if (output === null) return; patch("model", item.id, { input_cost_per_million: Number(input), output_cost_per_million: Number(output) }); }}>Editar custos</button><button onClick={() => { const ri=window.prompt("Custo de REFERÊNCIA de entrada por 1M tokens (USD)", item.reference_input_cost_per_million ?? item.input_cost_per_million); if(ri===null)return; const ro=window.prompt("Custo de REFERÊNCIA de saída por 1M tokens (USD)", item.reference_output_cost_per_million ?? item.output_cost_per_million); if(ro===null)return; const mode=window.prompt("Modo: FREE_TIER, PAID, RESERVED ou INTERNAL", item.billing_mode || "PAID"); if(mode===null)return; patch("model",item.id,{reference_input_cost_per_million:Number(ri),reference_output_cost_per_million:Number(ro),billing_mode:String(mode).toUpperCase()}); }}>Referência</button><button className={item.active ? "ai2-toggle on" : "ai2-toggle"} disabled={busy === `model:${item.id}`} onClick={() => patch("model", item.id, { active: !item.active })}>{item.active ? "Ativo" : "Inativo"}</button></div></td></tr>)}</tbody></table></div></section>}
      ...
      {tab === "Consumo" && <section className="ai2-panel"><header><div><span>PLENIUM AI · v0.9.3</span><h3>Centro de Custos e Rentabilidade</h3><p>Separa custo faturado do custo econômico de referência para medir o benefício real de Free Tier e preparar a escala.</p></div></header>
        <div className="ai2-metrics">
          <article><span>Operações no mês</span><strong>{data?.economics?.monthOperations || 0}</strong><small>execuções SUCCESS</small></article>
          <article><span>Custo faturado</span><strong>{usd(data?.economics?.billedMonthUsd, 6)}</strong><small>valor efetivamente cobrado</small></article>
          <article><span>Custo de referência</span><strong>{usd(data?.economics?.referenceMonthUsd, 6)}</strong><small>valor econômico sem subsídio</small></article>
          <article><span>Economia Free Tier</span><strong>{usd(data?.economics?.savingsMonthUsd, 6)}</strong><small>{pct(data?.economics?.savingsRate)} do custo de referência</small></article>
          <article><span>Média / operação</span><strong>{usd(data?.economics?.avgBilledPerOperation, 6)}</strong><small>custo faturado médio</small></article>
          <article><span>Projeção mensal</span><strong>{usd(data?.economics?.projectedBilledMonthUsd, 4)}</strong><small>ritmo atual de consumo</small></article>
        </div>
        <h4>Atribuição econômica do consumo</h4><div className="ai2-grid ai2-grid-3"><Metric label="Interno PLENIUM" value={`${data?.economics?.attributionSummary?.internal?.operations||0} operações`} hint="ROOT, benchmark e desenvolvimento"/><Metric label="Clientes" value={`${data?.economics?.attributionSummary?.clients?.operations||0} operações`} hint="consumo ligado a cliente/contrato"/><Metric label="Não atribuído" value={`${data?.economics?.attributionSummary?.unassigned?.operations||0} operações`} hint="deve tender a zero em produção"/></div><h4>Rentabilidade por cliente e plano</h4>
        <p className="ai2-help">Receita mensal vem dos contratos ativos. Custos de IA em USD são convertidos pela taxa operacional de {Number(data?.economics?.usdBrlRate||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})} BRL/USD.</p>
        <div className="ai2-table-wrap"><table><thead><tr><th>Cliente</th><th>Receita/mês</th><th>Operações IA</th><th>IA faturada</th><th>IA referência</th><th>% receita IA</th><th>Margem após IA</th></tr></thead><tbody>{(data?.economics?.clientEconomics||[]).map((item)=><tr key={item.organizationId}><td><strong>{item.clientName}</strong></td><td>{item.monthlyRevenueBrl>0?new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(item.monthlyRevenueBrl):"Sem contrato ativo"}</td><td>{item.operations}</td><td>{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:4}).format(item.billedBrl||0)}</td><td>{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:4}).format(item.referenceBrl||0)}</td><td>{item.aiRevenueShareReference==null?"—":pct(item.aiRevenueShareReference)}</td><td>{item.aiMarginReferenceBrl==null?"—":new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(item.aiMarginReferenceBrl)}</td></tr>)}{!(data?.economics?.clientEconomics||[]).length&&<tr><td colSpan="7" className="ai2-empty">Sem consumo atribuído a clientes neste mês.</td></tr>}</tbody></table></div>
        <h4>Simulação de escala</h4>
        <div className="ai2-table-wrap"><table><thead><tr><th>Operações/mês</th><th>Custo faturado estimado</th><th>Custo econômico estimado</th><th>Referência em BRL</th></tr></thead><tbody>{(data?.economics?.simulations||[]).map((item)=><tr key={item.operations}><td><strong>{Number(item.operations).toLocaleString("pt-BR")}</strong></td><td>{usd(item.billedUsd,4)}</td><td>{usd(item.referenceUsd,4)}</td><td>{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2,maximumFractionDigits:4}).format(item.referenceBrl||0)}</td></tr>)}</tbody></table></div>
        <h4>Economia por modelo</h4><div className="ai2-table-wrap"><table><thead><tr><th>Modelo</th><th>Provider</th><th>Operações</th><th>Tokens entrada</th><th>Tokens saída</th><th>Faturado</th><th>Referência</th><th>Economia</th></tr></thead><tbody>{(data?.economics?.byModel || []).map((item)=><tr key={item.model}><td><strong>{item.model}</strong></td><td>{item.provider}</td><td>{item.operations}</td><td>{Number(item.inputTokens||0).toLocaleString("pt-BR")}</td><td>{Number(item.outputTokens||0).toLocaleString("pt-BR")}</td><td>{usd(item.billedCostUsd,6)}</td><td>{usd(item.referenceCostUsd,6)}</td><td>{usd(Math.max(0,Number(item.referenceCostUsd||0)-Number(item.billedCostUsd||0)),6)}</td></tr>)}{!(data?.economics?.byModel||[]).length&&<tr><td colSpan="8" className="ai2-empty">Sem consumo no mês.</td></tr>}</tbody></table></div>
        <h4>Operações recentes</h4><div className="ai2-table-wrap"><table><thead><tr><th>Data</th><th>Operação</th><th>Nível</th><th>Modelo</th><th>Tokens</th><th>Custo</th><th>Status</th></tr></thead><tbody>{(data?.operations || []).slice(0,250).map((item) => <tr key={item.id}><td>{new Date(item.created_at).toLocaleString("pt-BR")}</td><td>{item.operation_type}</td><td>{levelName(item.requested_level)}</td><td>{modelMap[item.model_id]?.name || "—"}</td><td>{Number(item.input_tokens || 0) + Number(item.output_tokens || 0)}</td><td>{usd(item.cost_usd, 6)}</td><td><span className={`ai2-status ${item.status === "SUCCESS" ? "on" : "off"}`}>{item.status}</span></td></tr>)}{!(data?.operations || []).length && <tr><td colSpan="7" className="ai2-empty">Nenhuma operação real registrada ainda.</td></tr>}</tbody></table></div></section>}
