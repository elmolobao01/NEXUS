...
function categoryMatch(service, type) {
  ...
  if (type === "Domínios") return category === "DOMINIO" || provider.includes("registro.br") || String(service.resource_identifier || "").includes(".");
...
export default function InfrastructureSection() {
  ...
  async function submitService(event) {
    ...
    try {
      ...
      setForm(null); setMessage("Serviço cadastrado."); await refreshAlerts();
    } catch (error) { setMessage(error.message); }
  }
  ...
  function ServiceCards({ services, kind }) {
    return <div className="infra-noc-grid">
      ...
      {!services.length && <div className="infra-empty-card"><strong>Nenhum item cadastrado.</strong><p>Cadastre o primeiro serviço desta categoria para começar o monitoramento.</p><button type="button" className="root-button root-button-primary" onClick={() => setForm("service")}>+ Novo serviço</button></div>}
    </div>;
  }
  ...
  return <div className="infra-module">
    <section className="root-section-intro infra-intro">
      ...
      <div className="infra-header-actions"><button type="button" className="root-button root-button-secondary" onClick={refreshAlerts}>Recalcular alertas</button><button type="button" className="root-button root-button-primary" onClick={() => setForm("service")}>+ Novo serviço</button></div>
    </section>
    ...
    {form && <div className="infra-modal-backdrop" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)setForm(null)}}><section className="infra-modal" role="dialog" aria-modal="true"><header><div><span>PLENIUM ROOT</span><h3>{form === "service" ? "Novo serviço / ativo" : form === "plan" ? "Novo plano / alteração" : form === "account" ? "Lançamento manual" : "Registrar consumo"}</h3></div><button type="button" onClick={()=>setForm(null)} aria-label="Fechar">×</button></header>
      {form === "service" && <form onSubmit={submitService} className="infra-form"><label>Fornecedor<input name="provider" required placeholder="Ex.: Registro.br" /></label><label>Serviço / ativo<input name="name" required placeholder="Ex.: pleniumgestao.com.br" /></label><label>Categoria<select name="category"><option value="DOMINIO">Domínio</option><option value="BANCO_DADOS">Banco de dados</option><option value="HOSPEDAGEM">Hospedagem</option><option value="DESENVOLVIMENTO">Desenvolvimento / código</option><option value="API">API</option><option value="IA">IA</option><option value="EMAIL">E-mail</option><option value="OUTRO">Outro</option></select></label><label>Produto<select name="productCode"><option value="">Compartilhado</option>{PRODUCT_OPTIONS.map((p)=><option key={p}>{p}</option>)}</select></label><label>Identificador<input name="resourceIdentifier" placeholder="domínio, project ref, repositório..." /></label><label>URL de gestão<input name="managementUrl" type="url" placeholder="https://..." /></label><label>Início / contratação<input name="startedOn" type="date" /></label><label>Vencimento<input name="expiresOn" type="date" /></label><label>Próxima renovação<input name="renewalOn" type="date" /></label><label>Saúde inicial<select name="healthStatus"><option value="UNKNOWN">Não verificado</option><option value="ONLINE">Online</option><option value="ATTENTION">Atenção</option><option value="DEGRADED">Degradado</option><option value="OFFLINE">Offline</option></select></label><label className="infra-check"><input name="autoRenew" type="checkbox" /> Renovação automática</label><label className="infra-span-2">Observações<textarea name="notes" rows="3" /></label><div className="infra-form-actions"><button type="button" className="root-button root-button-secondary" onClick={()=>setForm(null)}>Cancelar</button><button className="root-button root-button-primary">Salvar serviço</button></div></form>}
