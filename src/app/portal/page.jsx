...
function CentralCliente() {
  ...
  return <>
    <section className="client-hero"><div><span className="hero-badge">CENTRAL DO CLIENTE</span><h2>Seu ecossistema PLENIUM</h2><p>Consulte sua contratação atual, benefícios, limites e amplie sua estrutura com novos produtos, módulos e Facilities.</p></div><div className="client-hero-mark"><img src="/branding/plenium-symbol.png" alt="" /></div></section>
...
export default function PortalPage() {
  ...
  return <main className="client-shell theme-educacao"><aside className="client-sidebar"><div className="client-brand"><div className="client-brand-mark"><img src="/branding/plenium-symbol.png" alt="PLENIUM" /></div><div><strong>PLENIUM</strong><span>Ecossistema de Soluções</span></div></div><div className="client-identity"><small>Organização</small><strong>{organization.nomeFantasia}</strong><span>{units[0]?.nome || "Unidade Matriz"}</span></div><nav className="client-navigation" aria-label="Áreas disponíveis">{modules.map((item) => <button key={item} type="button" className={activeModule === item ? "client-nav active" : "client-nav"} onClick={() => setActiveModule(item)}><span />{item}</button>)}</nav><div className="client-user"><span className="user-avatar user-avatar-brand"><img src="/branding/plenium-symbol.png" alt="" /></span><div><strong>Administrador</strong><small>Organização PLENIUM</small></div></div></aside>
  ...
  {activeModule === "Central do Cliente" ? <CentralCliente /> : activeModule === "Visão geral" ? <><section className="client-hero"><div><span className="hero-badge">AMBIENTE OPERACIONAL</span><h2>Bem-vindo ao PLENIUM</h2><p>Seu ambiente reúne estrutura, módulos e recursos autorizados para a realidade da sua organização.</p></div><div className="client-hero-mark"><img src="/branding/plenium-symbol.png" alt="" /></div></section><section className="client-metrics">{metrics.map(([label,value,detail]) => <article key={label}><span>{label}</span><strong>{value}</strong><small>{detail}</small></article>)}</section><section className="client-content-grid"><article className="client-panel"><div className="panel-heading"><div><span className="eyebrow">OPERAÇÃO</span><h3>Ambiente preparado</h3></div><span className="tag">ONLINE</span></div><div className="activity-list">{["Operação","Indicadores","Usuários","Relatórios"].map((item,index) => <div key={item}><span className="activity-index">0{index+1}</span><div><strong>{item}</strong><small>Estrutura disponível para a organização.</small></div><span className="activity-status">Ativo</span></div>)}</div></article><article className="client-panel"><div className="panel-heading"><div><span className="eyebrow">GESTÃO</span><h3>Atalhos administrativos</h3></div></div><ul className="licensed-list"><li><span>✓</span>Usuários e permissões</li><li><span>✓</span>Unidades e responsáveis</li><li><span>✓</span>Identidade e logomarca</li><li><span>✓</span>Configuração sob medida</li></ul></article></section></> : activeModule === "Usuários" ? <UsersManagement organizationId={organization.id} canInvite={["NEXUS_ROOT","NEXUS_ADMIN","CLIENT_ADMIN"].includes(profile)} /> : activeModule === "Configurações" ? <OrganizationSettings organization={organization} organizationId={organization.id} /> : <section className="client-panel client-module-placeholder"><span className="eyebrow">ÁREA OPERACIONAL</span><h2>{activeModule}</h2><p>Esta área permanece separada da Central do Cliente e receberá as funcionalidades específicas da organização e do segmento.</p></section>}
  </section></main>;
}
