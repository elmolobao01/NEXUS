...
export default function SetPasswordClient() {
  ...
  return <main className="nexus-activation-page"><section className="nexus-activation-card"><div className="nexus-activation-mark"><img src="/branding/plenium-symbol.png" alt="PLENIUM" /></div><span className="eyebrow">ATIVAÇÃO DE ACESSO</span><h1>Defina sua senha PLENIUM</h1><p>Seu administrador já configurou a organização, o perfil e as permissões iniciais. Crie sua senha para concluir a ativação.</p>{ready && accessToken ? <form onSubmit={submit}><label><span>Nova senha</span><input type="password" minLength="8" required value={password} onChange={(e)=>setPassword(e.target.value)} autoComplete="new-password"/></label><label><span>Confirmar senha</span><input type="password" minLength="8" required value={confirm} onChange={(e)=>setConfirm(e.target.value)} autoComplete="new-password"/></label><button className="primary-button" disabled={saving}>{saving ? "Ativando…" : "Ativar meu acesso"}</button></form> : null}{message ? <div className="client-feedback">{message}</div> : null}<small>O PLENIUM identificará automaticamente seu ambiente após o login.</small></section></main>;
}
